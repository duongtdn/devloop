...
│   ├── coder.md                 # Makes failing tests pass; commits when green (= no *new* failures — needs $ACCEPTED). Its green is provisional. Mode: spike (throwaway PoC, commits nothing)
...
- **A green suite is not always evidence**: the `reviewer` carries a `test-pass-insufficient` dimension for the bug class a passing suite cannot rule out (concurrency, shared-resource scoping, ordering, idempotency, resource lifecycle, reversibility of state-mutating artifacts). Findings there are argued from the code and types; "the tests pass" is not grounds to soften one, and `critique` may not drop one on those grounds. Test helpers and fixtures are reviewed at production rigour (a broken shared helper makes every test using it vouch for broken code), and code copied from `design.md` gets the same scrutiny as new code.
