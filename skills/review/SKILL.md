...
Every artifact devloop produces is the inner loop's testimony about itself: `context.md` Zone 2, `plan.md`, the state log, the test results. Reading them back to the human — even carefully, even with citations — is a replay of a self-report. **A replay cannot catch a self-consistent error**: the case where the code, its test, and its doc comment all agree with each other and disagree with the running system. That class of defect is real, it shipped green through every inner-loop gate, and it was caught here, by running the real thing.
...
- **Where the record is silent, look yourself — and never say a task ran clean on the strength of an empty flag list.** Every signal above is a **self-report**: it is what the run *knew* it was unsure about. Those signals track how *hard* the work was, not how likely it is to be wrong, and a task that was confidently wrong produces an empty list — easy and wrong is the ordinary way this happens, not an exotic one. So before calling anything clean, spend one pass on what the record cannot flag:
  ...
  - **first-of-its-kind structure** — if this task is the first in the codebase to establish a pattern (a module's internal layout, a new adapter shape), it becomes the exemplar handed to every later issue. Say so, and offer to walk it.
  - **comments that make architectural claims** — check one or two against the import graph. Here a false convention comment is not stale prose; it is an instruction to whatever builds the next module.

  A clean flag list means the run hit no surprises. It does not mean the design is right, and the opening must not imply that it does. Say the flag list is empty *and* say what you looked at yourself — those are two different statements and the human needs both.
...
4. **The task shipped** — its sprint-file checkbox is `[x]`. An **already-accepted** task is fine: `✓accepted` records a verdict, it does not seal the record — precondition 5 is what guards sealing. When the patch lands on an accepted task the acceptance **stands**: do not re-open it, do not strip the marker, and note in the Zone 2 entry that the patch post-dates it.
5. **The task belongs to the active sprint.** A task from a closed sprint is out of scope: its milestone is closed and its record is sealed — the retro is written and the tag is cut. Make it an issue.

**Why 4 stops at "shipped".** What makes this gate safe is the no-behaviour-change test, the clean tree, and a present human reading the real diff — none of which depends on when the task was ticked. Gating on `✓accepted` instead would exclude the most common and safest case the mechanism exists for: reviewing a sprint in order, by the time the human reaches the last issue every earlier one is accepted, and review is precisely when small cross-task defects first become visible. The realistic alternatives there are worse than patching — override the precondition (training the human to read the gate as advisory), file a backlog issue that costs more to schedule than to fix, or say nothing at all, which is the outcome that should worry us because it is invisible.

**A patch may touch a file owned by another issue in the same sprint.** Review reads the sprint as a whole, and that is often where a cross-task defect surfaces. Record the patch in Zone 2 of the issue **under review** (where the human found it) and comment on the issue that **owns the code** (where someone reading GitHub will look for why that file was touched twice).
...
**After the walkthrough, one whole-module pass — the only cross-diff look anything in devloop takes.** Every review before this one, inner and outer, was scoped to a single diff. Some defects are invisible at that scope *by construction*: when three issues each build one file of a module by mirroring the file before it, every diff conforms to the design and every review passes, and the flaw appears only when the finished set is laid side by side.

So for each module that gained **three or more files** this sprint, list its same-role files together — the adapters, the handlers, the resolvers — and ask one question: **do they have the same shape?** Compare the things that are cheap to compare and loud when they differ: constructor dependencies, what each imports, which of them holds logic and which just delegates. An outlier is not automatically wrong, and it is always worth asking about — the usual tell is a component that takes more than its siblings do, because the extra dependency is what let it absorb work that belongs somewhere else.

This is a question for the human, not a verdict of yours — it costs one comparison and it is the last chance anyone gets to see the module whole.

**Then**, if any **rework issues** were spawned, the sprint now has fresh unstarted work. Fork:
...
**A zero is ambiguous — read it with the gate's position in the sequence.** A gate that runs *late* sees only what every earlier gate left behind, so a low count there is expected and is not evidence the gate is idle. `validation` runs after two review passes have already cleaned the diff; a zero from it and a zero from a gate that genuinely never fires look identical in this table, and the difference matters because one of them is a candidate for deletion and the other is the last thing standing between a defect and `main`. When a late gate reads zero, say so in a clause — *"validation 0 (runs after review; sees only what review left)"* — rather than letting the number argue by itself.
